...
class EdgeConv(nn.Module):
    def __init__(self, k=12, input_size=64, output_size=64):
        super(EdgeConv, self).__init__()
        self.k = k
        self.k1_feaures = input_size
        self.output_size = output_size

        self.conv2_1 = nn.Conv2d(2 * input_size, output_size, 1)
        self.bn2_1 = nn.BatchNorm2d(output_size)


    def forward(self, x, knn_x):
        x = get_graph_feature(x, self.k, knn_x)
        x = self.conv2_1(x)
        x = self.bn2_1(x)
...
class iMeshSegNet(nn.Module):
    def __init__(self, num_classes=15, num_channels=15, with_dropout=True, dropout_p=0.5):
        ...
        # MLP-1 [64, 64]
        self.mlp1_conv1 = nn.Conv1d(self.num_channels, 64, kernel_size=1)
        ...
        # GLM-1 (EdgeConv)
        self.edgeconv_1 = EdgeConv(k=6, input_size=64, output_size=64)
        ...
        # GLM-2 (EdgeConv)
        self.edgeconv2_1 = EdgeConv(k=12, input_size=512, output_size=128)
        self.edgeconv2_2 = EdgeConv(k=6, input_size=512, output_size=128)
    ...
    def forward(self, x, knn_6=None, knn_12=None):
        ...
        # GLM-1 (EdgeConv)
        x = self.edgeconv_1(x_ftm, knn_12)
        ...
        # GLM-2 (EdgeConv)
        x_glm2 = torch.cat(( self.edgeconv2_1(x_mlp2, knn_12), 
                             self.edgeconv2_2(x_mlp2, knn_6)), dim=1)
